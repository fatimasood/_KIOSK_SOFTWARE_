...
class IndoorFormBackend {
  Database? _database;

  // Initialize the database
  Future<void> initDatabase() async {
    final databasePath = await getDatabasesPath();
    final path = join(databasePath, 'indoor_form.db');
    ...
    _database = await openDatabase(
      ...
      onUpgrade: (db, oldVersion, newVersion) async {
        if (oldVersion < 2) {
          ...
          await db.execute(
              'ALTER TABLE indoor_form ADD COLUMN lease_duration TEXT');
          await db
              .execute('ALTER TABLE indoor_form ADD COLUMN monthly_rent TEXT');
          await db.execute(
              'ALTER TABLE indoor_form ADD COLUMN security_deposit TEXT');
        }
      },
    );
  ...
  Future<void> saveFormData(Map<String, dynamic> formData) async {
    ...
    try {
      ...
      if (formData.containsKey('id')) {
        ...
        final id = await _database!.insert('indoor_form', formData);
        if (kDebugMode) {
  ...
  Future<void> fetchAllData() async {
    ...
    try {
      ...
      final data = await _database!.query('indoor_form');
      if (kDebugMode) {
